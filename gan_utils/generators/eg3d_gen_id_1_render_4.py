...
class StyledConv(nn.Module):
    def __init__(
        ...
        feat_multiplier = 1.25 if isconcat else 1
        self.activate = FusedLeakyReLU(int(out_channel*feat_multiplier), device=device)
...
class Generator(nn.Module):
    def __init__(
        ...
        self.feat_multiplier = 1.25 if isconcat else 1  # changes here
        ...
        self.to_rgb1 = ToRGB(
            int(self.channels[4]*self.feat_multiplier), 
            style_dim, 
            upsample=False, 
            device=device)
        ...
        for i in range(3, self.log_size + 1):
            ...
            self.to_rgbs.append(ToRGB(int(out_channel*self.feat_multiplier), style_dim, device=device))
    ...
    def forward(
        ...
        out, skip, latent, mix_zip = self.prepare(
            styles,
            inject_index=None,
            truncation=1,
            truncation_latent=None,
            input_is_latent=False,
            noise=None,)
        image = self.mixup(out, skip, latent, mix_zip)
        return image, latent if return_latents else None
...
class FullGenerator(nn.Module):
    def __init__(
        ...
        channels = {
            ...
            128: int(32 * channel_multiplier * narrow),
            256: int(16 * channel_multiplier * narrow),
            512: int(8 * channel_multiplier * narrow),
            1024: int(4 * channel_multiplier * narrow),
            2048: int(2 * channel_multiplier * narrow)
        }
