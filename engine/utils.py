...
def visualize_nerf_gan_outputs(
    nerf_out: dict, output_cache: List, viz: Visdom, visdom_env: str):
    ims_full = torch.cat(
        [
            nerf_out[imvar][0].detach().cpu().clamp(0.0, 1.0) 
            for imvar in ['rgb_pred', 'rgb_gt', 'rgb_ref']
        ],
        dim = 2)
    viz.image(
        ims_full,
        env=visdom_env,
        win="images_full",
        opts={"title": "pred | gt | ref"},
    )
    
def visualize_nerfgan_bgmask_outputs(
    nerf_out: dict, multi_images: dict, multi_masks: dict, output_cache: List, viz: Visdom, visdom_env: str):
    ims_full = torch.cat(
        [
            nerf_out[imvar][0].detach().cpu().clamp(0.0, 1.0) 
            for imvar in ['rgb_pred', 'rgb_gt', 'rgb_ref']
    ...
    multi_rendering = torch.cat(
        [
            F.interpolate(image, (512, 512))[0].detach().cpu().clamp(0.0, 1.0)  for image in multi_images.values()
        ],
        dim = 2
    )
    viz.image(
        multi_rendering,
        env=visdom_env,
        win="multi_resolution_generations",
        opts={"title": " | ".join(multi_images.keys())},
    )
    
    multi_isolation = torch.cat(
        [
            F.interpolate(mask, (512, 512))[0].detach().cpu().clamp(0.0, 1.0)  for mask in multi_masks.values()
        ],
        dim = 2
    )
    viz.image(
        multi_isolation,
        env=visdom_env,
        win="multi_resolution_isolation",
        opts={"title": " | ".join(multi_masks.keys())},
    )
...
def visualize_nerf_outputs(
    ...
    ims_full = torch.cat(
        ...
        dim=2,
    ) # ims_full.shape = torch.Size([3, 450, 1350])
    viz.image(
        ims_full,
        env=visdom_env,
        win="images_full",
...
class Stats:
    ...
    def reset(self) -> None:
        ...
        if self.verbose:
            log.info("stats: epoch %d - reset" % self.epoch)
    ...
    def hard_reset(self, epoch: int = -1) -> None:
        ...
        self.epoch = epoch
        if self.verbose:
            log.info("stats: epoch %d - hard reset" % self.epoch)
        self.stats = {}
        self.reset()
    ...
    def new_epoch(self) -> None:
        ...
        """
        if self.verbose:
            log.info("stats: new epoch %d" % (self.epoch + 1))
    ...
    def plot_stats(
        ...
        if plot_file is not None:
            log.info("Exporting stats to %s" % plot_file)
            ncol = 3
...
# def restore_checkpoint(cfg:DictConfig, 
...
#     model: torch.nn.Module
#         The model archetecture.
#     load_name: Optional[str]
#         The checkpoint name.
#     resume: bool
#         Whether to resume or start a new checkpoint. 
#     """
...
def visualize_nerfgan_bgmask_outputs(
    ...
    ims_full = torch.cat(
        [
            nerf_out[imvar][0].detach().cpu().clamp(0.0, 1.0) 
            for imvar in ['rgb_pred', 'rgb_gt', 'rgb_ref'] if imvar in nerf_out.keys()
        ],
        dim = 2)
    viz.image(
        ims_full,
