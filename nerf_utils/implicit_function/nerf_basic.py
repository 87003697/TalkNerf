...
def _xavier_init_weight(linear):
    """
...
class NeuralRadianceField(torch.nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        if not transparent_init:
            _xavier_init_weight(self.intermediate_linear)

        self.density_layer = torch.nn.Linear(n_hidden_neurons_xyz, 1)

        if not transparent_init:
            _xavier_init_weight(self.density_layer)
    ...
    def _get_densities_and_colors(
        ...
        if not self.return_feat:
            ...
            return rays_densities, [rays_colors, features]
