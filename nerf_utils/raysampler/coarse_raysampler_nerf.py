        ...
        **kwargs,
        ...
        if (camera_hash is None) and (not caching) and self.training:
            ...
            if camera_hash is not None:
                ...
                # full_ray_bundle.xys.shape = torch.Size([1, 800, 800, 2])
                # full_ray_bundle.origins.shape = torch.Size([1, 800, 800, 3])
                # full_ray_bundle.lengths.shape = torch.Size([1, 800, 800, 64])
